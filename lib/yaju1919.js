(function(window, undefined){
    ...
    var yaju1919 = {
        ...
        setBgImage: (function(){
            ...
            return function(url, cover){ // 背景画像を設定する。
                var p = yaju1919.setDefaultValue(cover,{
                    color: "white",
                    opacity: 0.8 // 透過度
                });
                if(!elm) elm = $("<div>").appendTo($("body"));
                elm.css({
                    zIndex: -114514,
    ...
    function _setCommonInput(p, elm, parentNode){
        ...
        elm.val(p.value).keypress(function(e){
    ...
    yaju1919.addInputNumber = function(parentNode, param){ // 数値入力欄を追加
        if(yaju1919.judgeType(param,"Object")){
            ['value', 'min', 'max'].forEach(function(v){
                if(yaju1919.judgeType(param[v],"String")) param[v] = Number(v);
            });
        }
        var p = yaju1919.setDefaultValue(param,{
            id: '', // HTML
    ...
    yaju1919.addSelect = function(parentNode, param){ // 選択肢を追加
        ...
        function updateSelect(){
            ...
            for(var k in p.list) $("<option>",{text:k}).val(p.list[k]).appendTo(elm);
            if(v) elm.val(v);
        }
        updateSelect();
